...
* Важно! map можно вызывать только на массивах, поэтому если мы получаем какую-то дату с сервера и нам надо превратить ее в элементы через map, то данные надо хранить в массиве, иначе это вызовет ошибку

* Также нельзя просто возвращать из компонента объект или переменную, это обязательно должна быть разметка. Пример:

``` jsx
return (
    {comments_list}  // <-- это объект, а не массив элементов JSX
)
```
